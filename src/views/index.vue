<template>
  <div class="page h-screen w-screen">
    ...
    <el-dialog
      v-model="showPublishReport"
      title="商品上传报告"
      width="90%"
      @closed="onReportClose"
    >
      <div class="statistics">
        共上传
        <span class="blue">{{ productsStore.candidateList.length }}</span>
        款商品，<span class="green">{{
          productsStore.candidateList.length - errorItems.length
        }}</span>
...
const queryList = async () => {
  try {
    const { list, page_info } = await query(
      `${baseUrl.value}?${stringify(allParams)}`,
    )
